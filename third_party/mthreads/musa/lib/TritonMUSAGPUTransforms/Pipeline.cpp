...
// This pass intentionally mirrors the public TritonGPU software pipeliner
// structure, but keeps MUSA-specific TME/barrier production, predication and
// SQMMA wait placement backend-local.
...
synthesizeMusaSqmmaSchedule(scf::ForOp forOp, int defaultNumStages) {
  ...
  if (!hasMusaSqmmaRoot(forOp))
    return failure();
  // All SQMMA loops need a coarse schedule before loop expansion. Descriptor
  // loops consume it in the MUSA-specific TME lowering below; plain tt.load
  // loops consume the same schedule in lowerLoads() to materialize
  // async_copy/multi-buffering. The later pipelineSqmma() step only decides
  // where SQMMA waits live once the load/compute overlap contract already
  // exists; it cannot create that overlap from scratch.
...
static void createAsyncCopy(scf::ForOp forOp, tt::LoadOp loadOp,
    ...
    if (!landing.sqmmaPlans.empty()) {
      ...
      if (auto contract =
        ...
        if (auto allocOp = landing.alloc.getDefiningOp())
          triton::musa::setSqmmaAttrsFromContract(allocOp, *contract);
  ...
  if (loadOp->use_empty()) {
    // All tensor users were replaced by direct memdesc views.
...
lowerLoads(scf::ForOp forOp, tt::CoarseSchedule &schedule,
  ...
  for (auto [numBuffers, loadGroup] : loadGroups) {
    // Match the upstream pipeliner contract even for a single buffer. Starting
    // from -1 ensures the first iteration advances into buffer 0 without being
    // treated as a wrap-around, so the first wait observes the initial phase
    // instead of a prematurely toggled one.
  ...
  for (auto &[numBuffers, loadGroup] : loadGroups) {
    ...
    if (phase) {
      ...
      // The software phase tracks the parity of the slot selected by the
      // extract counter. When the extract counter wraps, we revisit the same
      // hardware barrier slot, so the expected slot parity flips before the
      // next wait.
      loadGroup.yieldPhase =
          arith::SelectOp::create(builder, loc, cndExt, nextPhase, phase);
    }

    // Keep insert/extract bookkeeping as ordinary scalar SSA and let the
    // generic dependency closure place it later. The phase toggle is different:
    // it only feeds the next iteration, so without an explicit anchor it has
    // no scheduled in-iteration consumer and would be dropped from the coarse
    // schedule. Tie it to the consumer side that used the old phase.
